...
# REUSE: TranslationAdmin for models with translatable fields (django-modeltranslation)
# Requires: modeltranslation in INSTALLED_APPS and translation.py registration
try:
    from modeltranslation.admin import TranslationAdmin as BaseTranslationAdmin
except ImportError:
    BaseTranslationAdmin = UnfoldModelAdmin  # fallback if modeltranslation not installed
...
# ===========================
# Translation Admin
# REUSE: for models with en/ar translatable fields
# ===========================


class TranslationBaseAdmin(BaseTranslationAdmin, BaseAdmin):
    """
    Admin base for models with translatable fields (en/ar).
    Combines modeltranslation language tabs with Unfold theming.

    Usage:
        @admin.register(MyModel)
        class MyModelAdmin(TranslationBaseAdmin):
            list_display = ["__str__", "some_field"]

    Requires:
        - modeltranslation in INSTALLED_APPS (before admin)
        - translation.py registered for the model

    From ras-elbar-go/backend/common/unfold_admin_bases.py:206
    """
